...
<?php
	$sid = "3";
	$where = "WHERE page_id = '".$sid."'";
	
	$seoquery = $db->query("SELECT * FROM ".$ros->table('seo').$where." LIMIT 1");
	
	if($SeoB = $db->fetch_array($seoquery)){
		$page_title = $SeoB["ptitle"];
		$page_keywords = $SeoB["pkeyword"];
		$page_description = $SeoB["pdescription"];
	}
	include 'seo.php';
?>
...
<h1 class="comm-title ib" style="float:left; top:70px; margin-left:-270px; position:relative;">場合</h1>
<?php
$where = "";
